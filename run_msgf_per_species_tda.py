...
for i,species in enumerate(species_list):
    if not os.path.exists(f'{path_to_script}data/{dataset}/{species}/{species}_tda.mzid'):
        print(f'starting {species}',flush=True)
        subprocess.Popen(f'java -Xmx16G -jar {path_to_script}msgf/MSGFPlus.jar -s {path_to_script}data/{dataset}/{species}/{species}_tda.mgf -d {path_to_script}data/reference_dbs/{species.replace("1","").replace("2","").replace("3","")}.fasta -n 10 -t {ppm}ppm -inst {instruments[i]} -m {fragmentation_methods[i]} -tda 1'.split())
