...
if dataset == "pride":
    ...
    species_list = ['c_elegans','m_musculus','s_cerevisiae','h_sapiens1','h_sapiens2']
...
# 20 is the default by MSGF+
ppm = 20

for i,species in enumerate(species_list):
    if not os.path.exists(f'{path_to_script}data/{dataset}/{species}/{species}_20ppm.mzid'):
        print(f'starting {species}',flush=True)
        subprocess.Popen(f'java -Xmx8G -jar {path_to_script}msgf/MSGFPlus.jar -s {path_to_script}data/{dataset}/{species}/{species}_20ppm.mgf -d {path_to_script}data/reference_dbs/{species.replace("1","").replace("2","").replace("3","")}.fasta -n 10 -inst {instruments[i]} -m {fragmentation_methods[i]}'.split())
