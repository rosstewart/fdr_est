...
#                if not charge_line.startswith('CHARGE=1+'):
#                    print('charge error',charge_line)
#                    raise Exception
                
                #if 'scan=' not in next_line:
#                   print(next_line)
                #   continue

                scan = next_line.split('scan=')[1].split(' ')[0].strip()
                if scan not in scans:
                    scans[scan] = next_line
                else:
                    print(scan,'twice error')
                    print(scans[scan],'original',next_line,'new')
                    raise Exception
