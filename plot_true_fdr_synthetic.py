...
def get_fdrcurvs(species,xgrid):
    curvs = {}
...
def plot_avg_true_fdr(log_scale):

    if log_scale:
        xgrid = np.power(10, np.arange(-5, -1, 0.0001))
    else:
        xgrid = np.arange(0, 1.0, 0.0001)
    
    curvs = defaultdict(list)
    for species in species_list:
        sc = get_fdrcurvs(species,xgrid)
    ...
    for i in range(1,11):
        ...
        for method, mat in curvs.items():
            ...
            # for species_i, mat_species in mat:
            # print(mat.shape)
            # print(mat)
            
            mcurv = mat[i-1]#.mean(0)
            # curvstd = mat.std(0)
        ...
        mc = plt.plot([0,0.1 if log_scale else 1.0], [0,0.1 if log_scale else 1.0], linewidth=1, color='darkred',linestyle='dashed')
    #    lines.append(mc[0])
    #    legends.append('ground truth')
        
        ax.set_xlim(1e-3, 0.1 if log_scale else 1.0)
        ax.set_ylim(1e-3, 0.1 if log_scale else 1.0)
        ...
        if log_scale:
            plt.savefig(res_dir+f'fdrcmp_log_{i}.png', dpi=320)
            plt.savefig(res_dir+f'fdrcmp_log_{i}.pdf', dpi=320)
        else:
            plt.savefig(res_dir+f'fdrcmp_{i}.png', dpi=320)
            plt.savefig(res_dir+f'fdrcmp_{i}.pdf', dpi=320)


#%%
log_scale = False
plot_avg_true_fdr(log_scale=False)

log_scale = True
plot_avg_true_fdr(log_scale=True)
