...
#    print(estres.keys())
    ...
    s1 = np.array(s1)
    flags = fdr < 0.1
    plt.plot(fdr[flags], s1[flags])

#    plt.plot(estres['fdr'], np.log(estres['s1']))
    
def plot_fdr_thres_for_species(species):
    ...
    for method in method_list:
        ...
        plot_fdr_thres(estres['fdr'], estres['s1'])
        legends.append(algo_name_map[estres['algo']])
    
    tdares = tdajson[species]
    plot_fdr_thres(tdares['fdr'], tdares['s1'])
    legends.append('TDA')
...
#    plt.xlim(right=0.1)
    ...
    plt.savefig(fdrcurv_dir + 'fdrthres.png', dpi=320)
...
for species in species_list:
    plot_fdr_thres_for_species(species)
#    break
